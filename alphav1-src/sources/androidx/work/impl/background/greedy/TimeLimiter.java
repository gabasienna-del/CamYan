...
public final class TimeLimiter {
    ...
    public TimeLimiter(RunnableScheduler runnableScheduler, WorkLauncher launcher, long timeoutMs) {
        Intrinsics.checkNotNullParameter(runnableScheduler, "runnableScheduler");
        Intrinsics.checkNotNullParameter(launcher, "launcher");
    ...
    public final void track(final StartStopToken token) {
        ...
        Runnable stopRunnable = new Runnable() { // from class: androidx.work.impl.background.greedy.TimeLimiter$$ExternalSyntheticLambda0
        ...
        synchronized (this.lock) {
            this.tracked.put(token, stopRunnable);
        }
        this.runnableScheduler.scheduleWithDelay(this.timeoutMs, stopRunnable);
    }
    ...
    public final void cancel(StartStopToken token) {
        ...
        Intrinsics.checkNotNullParameter(token, "token");
        synchronized (this.lock) {
            it = this.tracked.remove(token);
        }
        if (it != null) {
            this.runnableScheduler.cancel(it);
        }
    }
}
