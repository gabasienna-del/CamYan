...
public final class WithLifecycleStateKt {
    ...
    private static final <R> Object withStateAtLeast$$forInline(LifecycleOwner $this$withStateAtLeast, Lifecycle.State state, Function0<? extends R> function0, Continuation<? super R> continuation) {
        $this$withStateAtLeast.getLifecycle();
        if (!(state.compareTo(Lifecycle.State.CREATED) >= 0)) {
            throw new IllegalArgumentException(("target state must be CREATED or greater, found " + state).toString());
        }
        Dispatchers.getMain().getImmediate();
            ...
            public void onStateChanged(LifecycleOwner source, Lifecycle.Event event) {
                ...
                if (event == Lifecycle.Event.INSTANCE.upTo(Lifecycle.State.this)) {
                    ...
                    try {
                        ...
                        m181constructorimpl = Result.m181constructorimpl(function02.invoke());
                    } catch (Throwable th) {
                        Result.Companion companion2 = Result.INSTANCE;
                        m181constructorimpl = Result.m181constructorimpl(ResultKt.createFailure(th));
                    }
                    continuation2.resumeWith(m181constructorimpl);
                    return;
                }
                if (event == Lifecycle.Event.ON_DESTROY) {
                    ...
                    continuation3.resumeWith(Result.m181constructorimpl(ResultKt.createFailure(new LifecycleDestroyedException())));
                }
            }
        };
        if (dispatchNeeded) {
            lifecycleDispatcher.mo1739dispatch(EmptyCoroutineContext.INSTANCE, new Runnable() { // from class: androidx.lifecycle.WithLifecycleStateKt$suspendWithStateAtLeastUnchecked$2$1
        ...
        co.invokeOnCancellation(new Function1<Throwable, Unit>() { // from class: androidx.lifecycle.WithLifecycleStateKt$suspendWithStateAtLeastUnchecked$2$2
            ...
            public final void invoke2(Throwable it) {
                if (CoroutineDispatcher.this.isDispatchNeeded(EmptyCoroutineContext.INSTANCE)) {
                    ...
                    coroutineDispatcher.mo1739dispatch(emptyCoroutineContext, new Runnable() { // from class: androidx.lifecycle.WithLifecycleStateKt$suspendWithStateAtLeastUnchecked$2$2.1
                        @Override // java.lang.Runnable
                        public final void run() {
