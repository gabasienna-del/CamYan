...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public NestedScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.mDifferentialMotionFlingTarget = new DifferentialMotionFlingTargetImpl();
        this.mDifferentialMotionFlingController = new DifferentialMotionFlingController(getContext(), this.mDifferentialMotionFlingTarget);
        ...
        switch (event.getKeyCode()) {
            case 19:
                if (event.isAltPressed()) {
                    boolean handled = fullScroll(33);
                    return handled;
                }
                boolean handled2 = arrowScroll(33);
                return handled2;
            case 20:
                if (event.isAltPressed()) {
                    boolean handled3 = fullScroll(130);
                    return handled3;
                }
                boolean handled4 = arrowScroll(130);
                return handled4;
            case 62:
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        velocityTrackerMotionEvent.offsetLocation(0.0f, this.mNestedYOffset);
        switch (actionMasked) {
            case 0:
                if (getChildCount() == 0) {
                    return false;
            ...
            case 2:
                ...
                if (activePointerIndex == -1) {
                    ...
                    int deltaY2 = deltaY - releaseVerticalGlow(deltaY, motionEvent.getX(activePointerIndex));
                    if (!this.mIsBeingDragged && Math.abs(deltaY2) > this.mTouchSlop) {
                        ...
                        this.mIsBeingDragged = true;
                        deltaY2 = deltaY2 > 0 ? deltaY2 - this.mTouchSlop : deltaY2 + this.mTouchSlop;
                    }
                    if (this.mIsBeingDragged) {
                        int x = (int) motionEvent.getX(activePointerIndex);
                        int scrollOffset = scrollBy(deltaY2, x, 0, false);
                        this.mLastMotionY = y - scrollOffset;
                        this.mNestedYOffset += scrollOffset;
                        break;
                    }
                }
                break;
        ...
        if (this.mVelocityTracker != null) {
            this.mVelocityTracker.addMovement(velocityTrackerMotionEvent);
        }
    ...
    private int scrollBy(int verticalScrollDistance, int x, int touchType, boolean isSourceMouseOrKeyboard) {
        int verticalScrollDistance2;
        int totalScrollOffset;
        if (touchType == 1) {
            startNestedScroll(2, touchType);
        }
        if (dispatchNestedPreScroll(0, verticalScrollDistance, this.mScrollConsumed, this.mScrollOffset, touchType)) {
            int verticalScrollDistance3 = verticalScrollDistance - this.mScrollConsumed[1];
            int totalScrollOffset2 = 0 + this.mScrollOffset[1];
            verticalScrollDistance2 = verticalScrollDistance3;
            totalScrollOffset = totalScrollOffset2;
        } else {
            verticalScrollDistance2 = verticalScrollDistance;
            totalScrollOffset = 0;
        }
        int initialScrollY = getScrollY();
        int scrollRangeY = getScrollRange();
        boolean canOverscroll = canOverScroll() && !isSourceMouseOrKeyboard;
        boolean hitScrollBarrier = overScrollByCompat(0, verticalScrollDistance2, 0, initialScrollY, 0, scrollRangeY, 0, 0, true) && !hasNestedScrollingParent(touchType);
        int scrollYDelta = getScrollY() - initialScrollY;
        int unconsumedY = verticalScrollDistance2 - scrollYDelta;
        this.mScrollConsumed[1] = 0;
        dispatchNestedScroll(0, scrollYDelta, 0, unconsumedY, this.mScrollOffset, touchType, this.mScrollConsumed);
        int totalScrollOffset3 = totalScrollOffset + this.mScrollOffset[1];
        int verticalScrollDistance4 = verticalScrollDistance2 - this.mScrollConsumed[1];
        int newScrollY = initialScrollY + verticalScrollDistance4;
        if (newScrollY < 0) {
            if (canOverscroll) {
                EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (-verticalScrollDistance4) / getHeight(), x / getWidth());
                if (!this.mEdgeGlowBottom.isFinished()) {
                    this.mEdgeGlowBottom.onRelease();
                }
            }
        } else if (newScrollY > scrollRangeY && canOverscroll) {
            EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, verticalScrollDistance4 / getHeight(), 1.0f - (x / getWidth()));
        ...
        if (hitScrollBarrier && touchType == 0 && this.mVelocityTracker != null) {
            this.mVelocityTracker.clear();
        }
        if (touchType == 1) {
    ...
    int consumeFlingInVerticalStretch(int unconsumedY) {
        ...
        if (unconsumedY > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            ...
            int consumed = Math.round(((-height) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, deltaDistance, 0.5f));
            if (consumed != unconsumedY) {
                this.mEdgeGlowTop.finish();
            }
            return unconsumedY - consumed;
        }
        if (unconsumedY < 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowBottom) != 0.0f) {
            float deltaDistance2 = (unconsumedY * FLING_DESTRETCH_FACTOR) / height;
            int consumed2 = Math.round((height / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, deltaDistance2, 0.5f));
    ...
    private float getSplineFlingDistance(int velocity) {
        double l = Math.log((Math.abs(velocity) * INFLEXION) / (this.mPhysicalCoeff * SCROLL_FRICTION));
        double decelMinusOne = DECELERATION_RATE - 1.0d;
        return (float) (this.mPhysicalCoeff * SCROLL_FRICTION * Math.exp((DECELERATION_RATE / decelMinusOne) * l));
    }
    ...
    public boolean onGenericMotionEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 8 && !this.mIsBeingDragged) {
            if (MotionEventCompat.isFromSource(motionEvent, 2)) {
                verticalScroll = motionEvent.getAxisValue(9);
                x = (int) motionEvent.getX();
                flingAxis = 9;
            } else if (MotionEventCompat.isFromSource(motionEvent, 4194304)) {
                float verticalScroll2 = motionEvent.getAxisValue(26);
                int x2 = getWidth() / 2;
                flingAxis = 26;
                x = x2;
                verticalScroll = verticalScroll2;
            } else {
                verticalScroll = 0.0f;
                x = 0;
                flingAxis = 0;
            ...
            if (verticalScroll != 0.0f) {
                int invertedDelta = (int) (getVerticalScrollFactorCompat() * verticalScroll);
                boolean isSourceMouse = MotionEventCompat.isFromSource(motionEvent, 8194);
                scrollBy(-invertedDelta, x, 1, isSourceMouse);
                if (flingAxis != 0) {
                    this.mDifferentialMotionFlingController.onMotionEvent(motionEvent, flingAxis);
                }
                return true;
            }
            return false;
    ...
    private boolean canOverScroll() {
        ...
        if (mode != 0) {
            return mode == 1 && getScrollRange() > 0;
        }
        return true;
    }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused != null && isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            int finalScrollDelta = direction == 130 ? scrollDelta : -scrollDelta;
            scrollBy(finalScrollDelta, 0, 1, true);
        }
    ...
    public void computeScroll() {
        ...
        boolean z = false;
        this.mScrollConsumed[1] = 0;
        dispatchNestedPreScroll(0, unconsumed, this.mScrollConsumed, null, 1);
        ...
        public boolean performAccessibilityAction(View host, int action, Bundle arguments) {
            ...
            switch (action) {
                ...
                case R.id.accessibilityActionScrollDown:
                    int viewportHeight = (height - nsvHost.getPaddingBottom()) - nsvHost.getPaddingTop();
                ...
                case R.id.accessibilityActionScrollUp:
                    int viewportHeight2 = (height - nsvHost.getPaddingBottom()) - nsvHost.getPaddingTop();
    ...
    class DifferentialMotionFlingTargetImpl implements DifferentialMotionFlingTarget {
        ...
        public boolean startDifferentialMotionFling(float velocity) {
            ...
            NestedScrollView.this.fling((int) velocity);
            return true;
        }
