...
public class FragmentTransitionSupport extends FragmentTransitionImpl {
    ...
    public Object cloneTransition(Object transition) {
        ...
        Transition copy = ((Transition) transition).mo70clone();
        return copy;
    }
    ...
    public boolean isSeekingSupported(Object transition) {
        boolean supported = ((Transition) transition).isSeekingSupported();
        if (!supported) {
            Log.v(FragmentManager.TAG, "Predictive back not available for AndroidX Transition " + transition + ". Please enable seeking support for the designated transition by overriding isSeekingSupported().");
        }
        return supported;
    }

    public Object controlDelayedTransition(ViewGroup sceneRoot, Object transition) {
        return TransitionManager.controlDelayedTransition(sceneRoot, (Transition) transition);
    }

    public void setCurrentPlayTime(Object transitionController, float progress) {
        TransitionSeekController controller = (TransitionSeekController) transitionController;
        if (controller.isReady()) {
            long time = ((float) controller.getDurationMillis()) * progress;
            if (time == 0) {
                time = 1;
            }
    ...
    public void animateToEnd(Object transitionController) {
        TransitionSeekController controller = (TransitionSeekController) transitionController;
        controller.animateToEnd();
    }

    public void animateToStart(Object transitionController, Runnable completeRunnable) {
        TransitionSeekController controller = (TransitionSeekController) transitionController;
    ...
    public void setListenerForTransitionEnd(Fragment outFragment, Object transition, CancellationSignal signal, Runnable transitionCompleteRunnable) {
        setListenerForTransitionEnd(outFragment, transition, signal, null, transitionCompleteRunnable);
    }

    public void setListenerForTransitionEnd(Fragment outFragment, Object transition, CancellationSignal signal, final Runnable cancelRunnable, final Runnable transitionCompleteRunnable) {
        final Transition realTransition = (Transition) transition;
        signal.setOnCancelListener(new CancellationSignal.OnCancelListener() { // from class: androidx.transition.FragmentTransitionSupport$$ExternalSyntheticLambda0
            @Override // androidx.core.os.CancellationSignal.OnCancelListener
            public final void onCancel() {
                FragmentTransitionSupport.lambda$setListenerForTransitionEnd$0(cancelRunnable, realTransition, transitionCompleteRunnable);
            }
        });
        realTransition.addListener(new Transition.TransitionListener() { // from class: androidx.transition.FragmentTransitionSupport.4
            @Override // androidx.transition.Transition.TransitionListener
    ...
    public static /* synthetic */ void lambda$setListenerForTransitionEnd$0(Runnable cancelRunnable, Transition realTransition, Runnable transitionCompleteRunnable) {
    ...
    public void setEpicenter(Object transitionObj, final Rect epicenter) {
        if (transitionObj != null) {
            Transition transition = (Transition) transitionObj;
            transition.setEpicenterCallback(new Transition.EpicenterCallback() { // from class: androidx.transition.FragmentTransitionSupport.5
                @Override // androidx.transition.Transition.EpicenterCallback
                public Rect onGetEpicenter(Transition transition2) {
