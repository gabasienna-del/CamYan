...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
    ...
    private BoundField createBoundField(Gson context, Field field, final Method accessor, String serializedName, TypeToken<?> fieldType, boolean serialize, final boolean blockInaccessible) {
        ...
        TypeAdapter<?> mapped2 = null;
        if (annotation != null) {
            mapped2 = this.jsonAdapterFactory.getTypeAdapter(this.constructorConstructor, context, fieldType, annotation, false);
        }
        boolean jsonAdapterPresent = mapped2 != null;
        if (mapped2 == null) {
            TypeAdapter<?> mapped3 = context.getAdapter(fieldType);
            mapped = mapped3;
        } else {
            mapped = mapped2;
        }
        final TypeAdapter<?> mapped4 = mapped;
        if (serialize) {
            if (jsonAdapterPresent) {
                typeAdapterRuntimeTypeWrapper = mapped4;
            } else {
                typeAdapterRuntimeTypeWrapper = new TypeAdapterRuntimeTypeWrapper<>(context, mapped4, fieldType.getType());
            }
            writeTypeAdapter = typeAdapterRuntimeTypeWrapper;
        } else {
            writeTypeAdapter = mapped4;
        }
        final TypeAdapter<?> typeAdapter = writeTypeAdapter;
        return new BoundField(serializedName, field) { // from class: com.google.gson.internal.bind.ReflectiveTypeAdapterFactory.2
            ...
            void readIntoArray(JsonReader reader, int index, Object[] target) throws IOException, JsonParseException {
                Object fieldValue = mapped4.read(reader);
    ...
    private FieldsData getBoundFields(Gson context, TypeToken<?> type, Class<?> raw, boolean blockInaccessible, boolean isRecord) {
        ...
        while (raw2 != Object.class) {
            ...
            while (i3 < length) {
                ...
                boolean serialize = includeField(field, z2);
                boolean deserialize2 = includeField(field, r7);
                if (!serialize && !deserialize2) {
                    ...
                    if (!isRecord) {
                        ...
                        if (accessor2.getAnnotation(SerializedName.class) != null && field.getAnnotation(SerializedName.class) == null) {
                            String methodDescription = ReflectionHelper.getAccessibleObjectDescription(accessor2, r7);
                    ...
                    String serializedName = fieldNames.get(r7);
                    i = i3;
                    i2 = length;
                    z = r7;
                    BoundField boundField = createBoundField(context, field, accessor, serializedName, TypeToken.get(fieldType), serialize, blockInaccessible2);
                    if (deserialize) {
                        for (String name : fieldNames) {
                            BoundField replaced2 = deserializedFields.put(name, boundField);
                            if (replaced2 != null) {
                                throw createDuplicateFieldException(raw, name, replaced2.field, field);
                            }
                        }
                    }
                    if (serialize && (replaced = serializedFields.put(serializedName, boundField)) != null) {
                        throw createDuplicateFieldException(raw, serializedName, replaced.field, field);
                    }
                }
                i3 = i + 1;
                length = i2;
