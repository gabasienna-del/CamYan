...
public abstract class FileHandle implements Closeable {
    ...
    public final long readNoCloseCheck(long fileOffset, Buffer sink, long byteCount) {
        ...
        long currentOffset2 = fileOffset;
        long targetOffset = fileOffset + byteCount;
        while (true) {
            if (currentOffset2 >= targetOffset) {
                currentOffset = currentOffset2;
                break;
            }
            Segment tail = sink.writableSegment$okio(readByteCount);
            byte[] bArr = tail.data;
            int i = tail.limit;
            long a$iv = targetOffset - currentOffset2;
            int b$iv = 8192 - tail.limit;
            currentOffset = currentOffset2;
            int $i$f$minOf = (int) Math.min(a$iv, b$iv);
            int readByteCount2 = protectedRead(currentOffset, bArr, i, $i$f$minOf);
            if (readByteCount2 == -1) {
                ...
                tail.limit += readByteCount2;
                currentOffset2 = readByteCount2 + currentOffset;
                sink.setSize$okio(sink.size() + readByteCount2);
                readByteCount = 1;
            }
        }
