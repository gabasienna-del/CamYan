...
public interface CompletableDeferred<T> extends Deferred<T> {
    ...
    public static final class DefaultImpls {
        ...
        public static <T> CoroutineContext plus(CompletableDeferred<T> completableDeferred, CoroutineContext context) {
            return Deferred.DefaultImpls.plus(completableDeferred, context);
        }

        @Deprecated(level = DeprecationLevel.ERROR, message = "Operator '+' on two Job objects is meaningless. Job is a coroutine context element and `+` is a set-sum operator for coroutine contexts. The job to the right of `+` just replaces the job the left of `+`.")
        public static <T> Job plus(CompletableDeferred<T> completableDeferred, Job other) {
