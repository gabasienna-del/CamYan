...
public class FakePhotoProvider extends ContentProvider {
    ...
    static { URI_MATCHER.addURI(AUTHORITY, "photo", CODE_PHOTO); }
    ...
    public ParcelFileDescriptor openFile(@NonNull Uri uri, @NonNull String mode)
        ...
        if (URI_MATCHER.match(uri) != CODE_PHOTO)
            throw new FileNotFoundException("Unknown uri "+uri);

        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(getContext());
        String img = prefs.getString("fake_photo_uri",null);

        if(img==null) throw new FileNotFoundException("No image selected");

        return getContext().getContentResolver().openFileDescriptor(Uri.parse(img),"r");
    }

    @Nullable @Override public Cursor query(Uri u,String[]p,String s,String[]a,String so){return null;}
